...
function ResourceToken({ resource, className = "resource-icon" }) {
  const meta = RESOURCE_META[resource];
  return (
    <span className={className} style={{ "--res": meta.color }} title={meta.label}>
      {meta.icon}
    </span>
  );
}

function CardStats({ player, resourceTotal, devTotal, cardTotal, discardRisk }) {
  return (
    <div className="stockpile">
      <span>{cardTotal} cards</span>
      <span className={discardRisk ? "warn" : ""}>{resourceTotal} res</span>
      <span>{devTotal} dev</span>
      <span>{5 - player.settlements_remaining}/5 settlements</span>
      <span>{4 - player.cities_remaining}/4 cities</span>
      <span>{15 - player.roads_remaining}/15 roads</span>
    </div>
  );
}
...
export default function PlayerPanel({ state, playerId, targetVp, byType = {}, onAction, onToggleMode, actionMode }) {
  ...
  const resourceTotal = totalResources(player.resources);
  const devTotal = totalDev(player);
  const cardTotal = resourceTotal + devTotal;
  const discardRisk = resourceTotal >= 10;
    ...
    return (
      <div className={`panel player-panel ${isTurn ? "active-turn" : ""}`}>
        <div className="player-head">
          ...
          <span className="vp-badge">{vp}/{targetVp} VP</span>
        </div>
        ...
        <div className="badges">
          ...
          {discardRisk && <span className="badge danger">Discard risk</span>}
        </div>

        <CardStats player={player} resourceTotal={resourceTotal} devTotal={devTotal} cardTotal={cardTotal} discardRisk={discardRisk} />
      </div>
    );
  }
  ...
  const clickCard = (c) => {
    ...
    if (c === "KNIGHT") {
      setPicker(null);
      onToggleMode("KNIGHT");
    } else if (c === "ROAD_BUILDING") {
      setPicker(null);
      onToggleMode("ROAD_BUILDING");
    } else if (c === "MONOPOLY") {
      if (actionMode) onToggleMode(null);
      setYopPick([]);
      setPicker((p) => (p === "MONOPOLY" ? null : "MONOPOLY"));
    } else if (c === "YEAR_OF_PLENTY") {
      if (actionMode) onToggleMode(null);
      setYopPick([]);
      setPicker((p) => (p === "YOP" ? null : "YOP"));
    }
  };

  const playMonopoly = (res) => {
    const action = (byType.PLAY_MONOPOLY ?? []).find((item) => item.payload.resource === res);
    if (action) {
      onAction(action);
      setPicker(null);
    }
  ...
  const yopAction =
    yopPick.length === 2
      ? (byType.PLAY_YEAR_OF_PLENTY ?? []).find((item) => multisetKey(item.payload.resources) === multisetKey(yopPick))
      : null;
  const takeYop = () => {
    if (yopAction) {
      onAction(yopAction);
  ...
  const held = DEV_CARD_ORDER.filter((card) => (player.dev_cards[card] || 0) + (player.new_dev_cards[card] || 0) > 0);
      ...
      <div className="dev-hand">
        ...
        {held.length === 0 ? (
          ...
          <div className="dev-hand-cards">
            {held.map((card) => {
              const count = (player.dev_cards[card] || 0) + (player.new_dev_cards[card] || 0);
              const canPlay = playable(card);
              const Tag = canPlay ? "button" : "span";
              return (
                <Tag
                  key={card}
                  className={`dev-card${canPlay ? " playable" : ""}${cardActive(card) ? " active" : ""}`}
                  title={canPlay ? `Play ${DEV_CARD_META[card].label}` : DEV_CARD_META[card].label}
                  onClick={canPlay ? () => clickCard(card) : undefined}
                >
                  <span className="dev-card-icon">{DEV_CARD_META[card].icon}</span>
        ...
        <div className="inset">
          ...
          <div className="res-chips">
            {RESOURCE_ORDER.map((resource) => (
              <button key={resource} className="res-chip" onClick={() => playMonopoly(resource)} title={RESOURCE_META[resource].label}>
                <ResourceToken resource={resource} className="res-chip-icon" />
              </button>
            ))}
          </div>
        </div>
      ...
      {picker === "YOP" && (byType.PLAY_YEAR_OF_PLENTY?.length ?? 0) > 0 && (
        <div className="inset">
          <p className="dev-hint">
            ...
            {yopPick.length > 0 && <strong>{yopPick.map((resource) => RESOURCE_META[resource].icon).join(" ")}</strong>}
          </p>
          <div className="res-chips">
            {RESOURCE_ORDER.map((resource) => (
              <button key={resource} className="res-chip" onClick={() => addYop(resource)} title={RESOURCE_META[resource].label}>
                <ResourceToken resource={resource} className="res-chip-icon" />
              </button>
            ))}
          </div>
